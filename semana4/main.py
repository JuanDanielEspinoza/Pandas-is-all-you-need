...
print(f"Registros originales: {len(df)} | Registros limpios: {len(df_limpio)}\n")
...
axes[0, 0].set_title('Distribución de Edad (Asimetría Izquierda)')
...
axes[0, 1].set_title('Nivel de Triglicéridos (Asimetría Derecha)')
...
# Gráfica 4: Presión Arterial limpia (Diagrama de caja)
# En matplotlib puro, los boxplots requieren un poco más de configuración para darles color
box = axes[1, 1].boxplot(df_limpio['Presion_Arterial_Sistolica'], vert=False, patch_artist=True)
for patch in box['boxes']:
    patch.set_facecolor('gold')
axes[1, 1].set_title('Presión Arterial Sistólica (Sensores Limpios)')
axes[1, 1].set_xlabel('Presión Arterial (mmHg)')
axes[1, 1].set_yticks([]) # Ocultar el '1' del eje Y que sale por defecto
